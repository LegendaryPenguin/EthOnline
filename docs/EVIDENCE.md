...
3:47, 1080p, captioned, and **rendered rather than screen-recorded** — `npm run record:render`
replays committed terminal casts frame by frame at 30 fps, so any frame can be diffed against the
bytes the command actually printed. Raw material in `docs/evidence/screens/` (ten screenshots, both
themes, regenerated by `npm run capture:ui`) and `docs/evidence/casts/`. Script and mechanism:
`docs/VIDEO.md`.

Shows, in order: the headline finding; one query document spanning five protocols; the TEE
simulation logs; the backtest result with lead time (the operating-point table from
`docs/evidence/phase6-early-warning.md` — 3 of 5 episodes at a 24h median lead at 20% FPR, and
0 of 5 at the primary 10% row); the cascade animation driven by the shock slider; and
`GuardedVault` reacting to the signal on-chain.
