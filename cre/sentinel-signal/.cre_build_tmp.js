...
function normalizeSide(raw) {
  ...
  if (s === "BORROWER")
    return "BORROWER";
  if (s === "COLLATERAL" || s === "LENDER" || s === "SUPPLIER")
    return "COLLATERAL";
  throw new Error(`Unrecognized PositionSide: ${raw}`);
}
function buildPriceIndex(markets) {
  const best = new Map;
  for (const m of markets) {
    ...
    if (!incumbent || tvl > incumbent.tvl)
      best.set(id, { price, tvl });
  }
  return new Map([...best].map(([id, v]) => [id, v.price]));
}
function toTokenUnits(balance, decimals) {
...
function receiptExchangeRate(market, assetId, assetDecimals) {
  const outputId = market.outputToken?.id.toLowerCase();
  if (!outputId || outputId !== assetId)
    return null;
...
function normalizePosition(raw, protocol, prices) {
  ...
  const price = sameAsset && usableMarketPrice ? marketPrice : receiptRate !== null && usableMarketPrice ? marketPrice * receiptRate : prices.get(assetId) ?? 0;
  if (price <= 0)
    return null;
  return {
    id: raw.id,
    ...
    ...receiptRate !== null ? { underlyingAssetId: raw.market.inputToken.id.toLowerCase() } : {}
  };
}
var emptyProtocol = () => ({
  collateralUsd: 0,
...
function measureProtocol(positions, floor) {
...
function correlatedFloor(positions, emode) {
  const assets = new Set;
  for (const p of positions) {
    if (p.valueUsd <= 0)
      continue;
    if (p.side === "COLLATERAL" && p.liquidationThreshold <= 0)
      continue;
  ...
  for (const group of emode.groups) {
    ...
    if (all)
      return emode.threshold;
  }
  return 0;
}
function buildExposure(account, positions, emode) {
  const positionsByProtocol = new Map;
  const collateralByAsset = {};
  for (const p of positions) {
    const bucket = positionsByProtocol.get(p.protocol);
    if (bucket)
      bucket.push(p);
    else
      positionsByProtocol.set(p.protocol, [p]);
...
function classify(b) {
  if (b.debtUsd > 0 && b.healthFactor < 1)
    return "contradicted";
  if (b.unknownThresholdUsd > 0)
    return "incomplete";
  return "ok";
}
var RANK = { ok: 0, incomplete: 1, contradicted: 2 };
var weaker = (a, b) => RANK[b] > RANK[a] ? b : a;
var WEIGHT_KEYS = ["concentration", "leverage", "distress"];
var WEIGHT_SUM_EPSILON = 0.000000001;
function parseRiskPolicy(json) {
  let raw;
  try {
    raw = JSON.parse(json);
  } catch {
  ...
  if (shocks.length === 0)
    throw new Error("risk policy: shocks must not be empty");
  for (const s of shocks) {
    if (s <= 0 || s >= 1)
      throw new Error(`risk policy: shock ${s} outside (0,1)`);
  }
  ...
  for (const [asset, beta] of Object.entries(betaRaw)) {
  ...
  for (const k of WEIGHT_KEYS) {
    ...
    if (v < 0)
      throw new Error(`risk policy: weights.${k} is negative`);
    weights[k] = v;
    sum += v;
  }
  ...
  if (!Number.isInteger(kAnonymity))
    throw new Error("risk policy: kAnonymity must be an integer");
  if (kAnonymity < 2)
    throw new Error("risk policy: kAnonymity below 2 suppresses nothing");
  return {
    shocks,
    assetBeta,
...
function parseEmode(v) {
  if (typeof v !== "object" || v === null)
    throw new Error("risk policy: emode must be an object");
  const o = v;
  const threshold = positive(o.threshold, "emode.threshold");
  if (threshold >= 1)
    throw new Error(`risk policy: emode.threshold ${threshold} is not below 1`);
  if (!Array.isArray(o.groups))
    throw new Error("risk policy: emode.groups must be an array");
  const groups = [];
  const seen = new Map;
  for (const [i, group] of o.groups.entries()) {
    ...
    for (const asset of group) {
      if (typeof asset !== "string" || !/^0x[0-9a-f]{40}$/.test(asset)) {
        throw new Error(`risk policy: emode.groups[${i}] contains a non-lowercased address: ${String(asset)}`);
      }
...
function betaFor(policy, assetId) {
  return policy.assetBeta[assetId] ?? policy.defaultBeta;
}
function finite(v, field) {
...
function positive(v, field) {
  ...
  if (n <= 0)
    throw new Error(`risk policy: ${field} must be positive`);
  return n;
}
function numberArray(v, field) {
  if (!Array.isArray(v))
    throw new Error(`risk policy: ${field} must be an array`);
  return v.map((x, i) => finite(x, `${field}[${i}]`));
}
var SIGNAL_VERSION = "sentinel-signal/1";
function aggregateSignal(input) {
  ...
  let reportedTotal = 0;
  for (const v of Object.values(input.reportedDebtUsd))
    reportedTotal += v;
  const share = (n) => evaluableDebtUsd > 0 ? n / evaluableDebtUsd : 0;
  ...
  const signal = {
    ...
    systemicRiskScore: 100 * clamp01(policy.weights.concentration * share(multiProtocolDebtUsd) + policy.weights.leverage * share(leveredDebtUsd) + policy.weights.distress * share(worstShock?.distressedDebtUsd ?? 0))
  };
  assertAggregateOnly(signal);
  return signal;
}
function shockedDistress(e, shock, policy) {
  const weighted = {};
  const debt = {};
  for (const p of e.positions) {
    ...
    const threshold = Math.max(p.liquidationThreshold, e.byProtocol[p.protocol]?.emodeThreshold ?? 0);
    weighted[p.protocol] = (weighted[p.protocol] ?? 0) + p.valueUsd * (1 - decline) * threshold;
  }
  let hit = 0;
  for (const [protocol, d] of Object.entries(debt)) {
    if (d > 0 && (weighted[protocol] ?? 0) < d)
      hit += d;
  }
  return hit;
}
function borrowingProtocols(e) {
  const seen = new Set;
  for (const p of e.positions) {
    if (p.side === "BORROWER" && p.valueUsd > 0)
      seen.add(p.protocol);
  }
  return [...seen].sort();
}
function groupByAccount(positions, emode) {
...
var ADDRESS_LIKE = /0x[0-9a-fA-F]{40}/;
function assertAggregateOnly(signal) {
...
function positionFields(schemaVersion) {
...
function enclaveBootstrapQuery(atBlock) {
...
var ENCLAVE_BOOTSTRAP_QUERY = enclaveBootstrapQuery();
var MARKET_ID = /^[0-9a-zA-Z:_.-]+$/;
function topPositionsQuery(schemaVersion, marketIds, perMarket, atBlock) {
  if (marketIds.length === 0)
    throw new Error("topPositionsQuery: no markets");
...
function completeBooksQuery(schemaVersion, atBlock) {
  const at = atBlockArg(atBlock);
  return `
...
function collectAliasedPositions(data) {
  const out = [];
...
function pad3(hexOrBytes, { dir, size = 32 } = {}) {
  if (typeof hexOrBytes === "string")
    return padHex3(hexOrBytes, { dir, size });
  return padBytes3(hexOrBytes, { dir, size });
}
function padHex3(hex_, { dir, size = 32 } = {}) {
  if (size === null)
    return hex_;
  const hex = hex_.replace("0x", "");
  if (hex.length > size * 2)
...
function padBytes3(bytes, { dir, size = 32 } = {}) {
  if (size === null)
    return bytes;
  if (bytes.length > size)
    throw new SizeExceedsPaddingSizeError3({
...
function assertSize3(hexOrBytes, { size }) {
  if (size3(hexOrBytes) > size)
...
function keccak2562(value, to_) {
  const to = to_ || "hex";
  const bytes = keccak_2562(isHex3(value, { strict: false }) ? toBytes4(value) : value);
  if (to === "bytes")
    return bytes;
  return toHex2(bytes);
}
...
function encodeArray(value, { length, param }) {
  ...
  if (dynamic || dynamicChild) {
    ...
    if (dynamic) {
      const length = numberToHex2(preparedParams.length, { size: 32 });
      return {
        dynamic: true,
        encoded: concatHex2([length, data])
      };
    }
    if (dynamicChild)
...
function encodeBool(value) {
  if (typeof value !== "boolean")
    throw new BaseError5(`Invalid boolean value: "${value}" (type: ${typeof value}). Expected: \`true\` or \`false\`.`);
  return { dynamic: false, encoded: padHex3(boolToHex2(value)) };
}
...
function encodeString(value) {
  ...
  for (let i = 0;i < partsLength; i++) {
    parts.push(padHex3(slice2(hexValue, i * 32, (i + 1) * 32), {
      dir: "right"
    }));
  }
  return {
...
var SIGNAL_ABI_PARAMS = "string version, uint64 asOfBlock, uint32 borrowersObserved, uint256 debtUsd6, " + "uint256 evaluableDebtUsd6, uint256 multiProtocolDebtUsd6, uint16 multiProtocolShareBps, " + "uint16 leveredShareBps, uint16 worstShockBps, uint256 worstShockDistressedDebtUsd6, " + "uint16 systemicRiskScoreBps, uint16 couplingBuckets, uint16 suppressedBuckets, " + "uint32 emodeInferredBorrowers, uint256 emodeInferredDebtUsd6";
function encodeSignal(signal) {
  const worst = signal.shockLadder[signal.shockLadder.length - 1];
  return encodeAbiParameters2(parseAbiParameters2(SIGNAL_ABI_PARAMS), [
    signal.version,
...
function asOfBlock(signal) {
  const numbers = Object.values(signal.blocks);
  return numbers.length === 0 ? 0 : Math.min(...numbers);
}
function usd6(usd) {
  if (!Number.isFinite(usd) || usd < 0)
    throw new Error(`usd6: refusing to encode ${usd}`);
...
function bps(fraction) {
  if (!Number.isFinite(fraction) || fraction < 0)
    throw new Error(`bps: refusing ${fraction}`);
  return Math.min(1e4, Math.round(fraction * 1e4));
}
var deploymentSchema = objectType({
  key: stringType().regex(/^[a-z0-9-]+$/, "deployment key must be a slug, never an address"),
  label: stringType(),
  subgraphId: stringType(),
  schemaVersion: stringType()
});
var configSchema = objectType({
  schedule: stringType(),
  gatewayBaseUrl: stringType(),
  apiKeySecretId: stringType(),
  policySecretId: stringType(),
  deployments: arrayType(deploymentSchema).min(1),
  marketsPerDeployment: numberType().int().positive(),
  marketsSampled: numberType().int().positive(),
  positionsPerMarket: numberType().int().positive(),
  candidatesPerDeployment: numberType().int().positive(),
  candidateAccounts: numberType().int().positive(),
  completeBooksPageSize: numberType().int().positive(),
  maxBlockLag: numberType().int().nonnegative(),
  httpCallBudget: numberType().int().positive(),
  logDiagnostics: booleanType()
});
var MAX_DEBT_RECONCILIATION_RATIO = 1.25;
...
var onCronTrigger = (runtime) => {
  ...
  for (const d of config.deployments) {
    try {
      bootstrap.set(d.key, gql(d, ENCLAVE_BOOTSTRAP_QUERY, { markets: config.marketsPerDeployment }));
    } catch (err) {
  ...
  let head = 0;
  for (const b of bootstrap.values())
    head = Math.max(head, b._meta.block.number);
  const live = [];
  const blocks = {};
  for (const d of config.deployments) {
  ...
  if (live.length === 0)
    throw new Error(`every deployment lags head ${head}`);
  const allMarkets = [];
  for (const d of live)
    allMarkets.push(...bootstrap.get(d.key)?.markets ?? []);
  const prices = buildPriceIndex(allMarkets);
  const sampled = [];
  const sampledBy = new Map;
  const discovered = [];
  for (const d of live) {
    const markets = (bootstrap.get(d.key)?.markets ?? []).filter((m) => Number(m.totalDepositBalanceUSD) > 0 || Number(m.totalBorrowBalanceUSD) > 0).slice(0, config.marketsSampled);
    if (markets.length === 0) {
      notes.push(`${d.key} excluded: no market with a book`);
      continue;
    }
    try {
      const data = gql(d, topPositionsQuery(d.schemaVersion, markets.map((m) => m.id), config.positionsPerMarket), {});
      const positions = [];
      for (const raw of collectAliasedPositions(data)) {
      ...
      if (ratio !== undefined && ratio > MAX_DEBT_RECONCILIATION_RATIO) {
        notes.push(`${d.key} excluded at discovery: sampled debt is ${ratio.toFixed(1)}x the ` + "protocol-reported total, so its position mappings disagree with its own totals");
        delete blocks[d.key];
        continue;
      }
  ...
  const candidates = largestBorrowers(sampled.filter((p) => nominated.has(p.account)), config.candidateAccounts);
  if (candidates.length === 0)
    throw new Error("discovery found no priced borrower");
  const complete = [];
  const reportedDebtUsd = {};
  for (const d of discovered) {
    let rows;
    try {
      rows = gql(d, completeBooksQuery(d.schemaVersion), {
        accounts: candidates,
    ...
    if (rows.length >= config.completeBooksPageSize) {
      throw new Error(`${d.key}: complete-book page filled at ${config.completeBooksPageSize} rows, so books ` + "are truncated and health factors would be meaningless; lower candidateAccounts");
    }
    const positions = [];
    ...
    const ratio = debtRatio(positions, bootstrap.get(d.key));
    if (ratio !== undefined && ratio > MAX_DEBT_RECONCILIATION_RATIO) {
      notes.push(`${d.key} excluded: sampled debt is ${ratio.toFixed(1)}x the ` + "protocol-reported total, so its position mappings disagree with its own totals");
      delete blocks[d.key];
      continue;
    }
    reportedDebtUsd[d.key] = Number(bootstrap.get(d.key)?.lendingProtocols[0]?.totalBorrowBalanceUSD ?? 0);
  ...
  if (config.logDiagnostics) {
    runtime.log(`enclave: ${signal.borrowersObserved} borrowers, score ` + `${signal.systemicRiskScore.toFixed(1)}, ${signal.coupling.length} coupling buckets, ` + `${signal.suppressedBuckets.length} suppressed, ${notes.length} deployment notes`);
    for (const n of notes)
      runtime.log(`enclave: ${n}`);
  }
...
function summarize(signal, notes) {
  const worst = signal.shockLadder[signal.shockLadder.length - 1];
  return [
...
function makeGatewayClient(runtime, apiKey) {
  const config = runtime.config;
  const http = new cre.capabilities.HTTPClient;
  let calls = 0;
  return function gql(deployment, document, variables) {
    if (++calls > config.httpCallBudget) {
      throw new BudgetExhausted(`HTTP call budget of ${config.httpCallBudget} exhausted at ${deployment.key}; ` + "reduce deployments or marketsSampled");
    }
    ...
    if (body.errors && body.errors.length > 0) {
      throw new Error(`GraphQL error for ${deployment.key}: ${body.errors.map((e) => e.message).join("; ")}`);
    }
    if (!body.data)
      throw new Error(`gateway returned no data for ${deployment.key}`);
    return body.data;
  };
}
function largestBorrowers(positions, n) {
  const debt = new Map;
  for (const p of positions) {
    if (p.side !== "BORROWER" || p.valueUsd <= 0)
      continue;
  ...
  return [...debt].sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0])).slice(0, n).map(([account]) => account);
}
var message = (err) => err instanceof Error ? err.message : String(err);
function debtRatio(positions, bootstrap) {
